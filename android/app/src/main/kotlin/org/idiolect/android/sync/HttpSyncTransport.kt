...
/**
 * [SyncTransport] over HTTP(S) to the user's PC (`idiolect-sync-server`'s `POST
 ...
 *
 * Under TLS (the default) the persisted [pin] is applied via [applyPinning], so the bearer
 * token and the user's learning data only cross a connection to the pinned cert; a cleartext
 * (`--no-tls`) endpoint passes a null [pin].
 */
