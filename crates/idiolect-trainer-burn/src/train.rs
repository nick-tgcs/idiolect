...
/// `[sot, no_timestamps, …text…, eot]`: position `i` predicts `tokens[i+1]`,
/// and the prompt's own continuation (sot → no_timestamps) is not trained.
pub fn sequence_loss<B: AutodiffBackend>(
    ...
    assert!(tokens.len() >= 3, "need at least one trained position");
    let logits = runtime.decoder_logits(tokens, encoder_output, Some(lora));
    let n = tokens.len();
    let rows = logits.narrow(0, 1, n - 2);
    let targets: Vec<i32> = tokens[2..n].to_vec();
...
pub fn train_step<B: AutodiffBackend>(
    ...
    let loss = sequence_loss(runtime, encoder_output, tokens, lora);
    let value = loss
        .clone()
        .into_data()
        .to_vec::<f32>()
